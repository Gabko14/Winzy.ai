...
#
# Out of scope (derived data):
#   Notifications and Activity Feed entries are created by the backend's
#   in-process event handlers reacting to the actions above. This script does
#   NOT verify their output. If you need to test those, check the
#   /notifications and /activity/feed endpoints after seeding.
#
...
cat <<'SCENARIOS'
  ...
  Note: Notifications and Activity Feed entries are generated by the
  backend's in-process event handlers. They are NOT verified by
