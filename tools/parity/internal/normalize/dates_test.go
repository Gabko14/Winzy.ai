...
func TestStableSortFeedItems_TiedCreatedAt(t *testing.T) {
	golden := map[string]any{
		"items": []any{
			map[string]any{"createdAt": "{{timestamp}}", "eventType": "friend.request.accepted", "actorId": "b", "id": "2"},
			map[string]any{"createdAt": "{{timestamp}}", "eventType": "friend.request.accepted", "actorId": "a", "id": "1"},
		},
	}
	...
	if diffs := Diff("$", sg, sa); len(diffs) != 0 {
		t.Fatalf("expected equal after stable sort, got %v", diffs)
	}
