...
- **Reviewers must test, not just read code.** Run the quality gates yourself (`dotnet test`, `npm test`, `tsc --noEmit`). If the stack is running, use the Playwright MCP to actually navigate the app and verify the feature works. A review that only reads code misses runtime bugs.
